          ...
          <>
            ...
            <div className='application-metrics__ChartContainerFlex'>
              {row?.graphs?.map((graph: any) => {
                const url = `https://localhost:8080/api/v1/application/metricsqueryruns/resource?resourceName=${row.name}-${graph.name.replaceAll("_", "-")}&namespace=${namespace}&name=${application_name}&version=v1&kind=ConfigMap`
                return (
                  <ChartWrapper
