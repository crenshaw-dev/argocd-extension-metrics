...
export const ChartWrapper = ({
  ...
  }: { data: [PrometheusResponse] & WavefrontResponse, groupBy: string, yFormatter?: (arg0: number) => number, xFormatter?: (arg0: number) => number }) => {
    ...
    if(data?.granularity){
      ...
      data?.map((obj: PrometheusResponse) => {
        ...
        obj?.values?.map((kp: [any, any], i: number) => {
          if (obj?.values?.length && (metricObj.data?.[i - 1]?.[0] < (kp[0] - 61))) {
            metricObj.data.push({
